...
const v = (participantId: string, value: VoteValue): CastVote => ({ participantId, value });
...
describe('decideByDecisionType', () => {
  ...
  it('maioria qualificada: usa percentual arredondado pra cima', () => {
    const c = countVotes(
      [v('a', YES), v('b', YES), v('c', YES), v('d', YES), v('e', NO)],
      ['a', 'b', 'c', 'd', 'e'],
    );
    expect(
      decideByDecisionType(
        cfg({ decisionType: DecisionType.QUALIFIED_MAJORITY, qualifiedMajorityPercentage: 66.67 }),
        c,
      ),
